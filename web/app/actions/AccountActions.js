...
class AccountActions {
     ...
     *  be linked.
     */
    upgradeAccount(account_id, lifetime,policy_flag,policy_hash_code) {
        // Set the fee asset to use
        ...
        tr.add_type_operation("account_upgrade", {
            ...
            "policy_hash_code":policy_hash_code
        });
        return WalletDb.process_transaction(tr, null, true);
    }
    /**
     *  TODO:  to upload ipfs hashcode 
     */
    policy(account_id,file_name,file_format,file_size,policy_hash_code) {
        let fee_asset_id = accountUtils.getFinalFeeAsset(account_id, "policy");

        var tr = wallet_api.new_transaction();
        tr.add_type_operation("policy", {
            "fee": {
                amount: 0,
                asset_id: fee_asset_id
            },
   ...
   {
        return (dispatch) => {
                return AccountApi.get_operations(block_num, trx_in_block)
                .then(result => {
                    dispatch({accounts: result});
                });
